...
const updateMesa = async (req, res) => {
    const { numero, capacidad, disponibles, comensales } = req.body;

    try {
        const mesa = await Mesa.findByIdAndUpdate(
            req.params.mesaId,
            { numero, capacidad, disponibles, comensales },
            { new: true }
        );
        if (!mesa) {
            return res.status(404).send({ message: "Mesa no encontrada" });
...
const getMesasDisponibles = async (req, res) => {
    try {
        const mesasDisponibles = await Mesa.find({ disponibles: true });
...
const updateDisponibilidadMesa = async (req, res) => {
    ...
    try {const Mesa = require('../models/mesa.model');
